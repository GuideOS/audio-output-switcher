...
//   - Integration der Funktionalität von audio-output-switcher@ktt73
//     direkt in dieses Applet – kein separates zweites Panel-Applet nötig.
//   - Neue Hilfsfunktion _audioSpawnSync() für synchrone pactl-Aufrufe.
//   - Neuer Untermenüpunkt „Audio Output" im Haupt-Popup-Menü:
//       · Listet alle verfügbaren PulseAudio/PipeWire-Sinks inkl. Ports auf.
//       · Erkennt und kennzeichnet Gerätetypen (Bluetooth, HDMI/DisplayPort,
//         S/PDIF, USB-Audio, Line-out, Kopfhörer, Lautsprecher).
//       · Zeigt den aktiven Ausgang mit ✓ an.
//       · Verschiebt beim Umschalten alle laufenden sink-inputs automatisch.
//       · Filtert interne Monitor-Sinks (PipeWire/PulseAudio) heraus.
//   - Untermenü-Label wird alle 4 Sekunden sowie beim Öffnen des Menüs
//     aktualisiert (erkennt z. B. neu verbundene Bluetooth-Geräte).
//   - Korrekte Aufräumlogik (_audioOutputTimeout) in
...
function _audioSpawnSync(cmd) {
    try {
        ...
        global.logError("sound@cinnamon.org: _audioSpawnSync – " + e);
        return null;
    }
}
//Fork Ende
...
class CinnamonSoundApplet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instanceId) {
        ...
        this._audioOutputTimeout = Mainloop.timeout_add_seconds(4, () => {
    ...
    _parseAudioSinks() {
        let output = _audioSpawnSync("pactl list sinks");
        if (!output) return [];

        let sinks = [], current = null, inPorts = false;
        ...
        if (current) sinks.push(current);
        return sinks.filter(s => !s.name.endsWith(".monitor") && !s.name.includes(".monitor."));
    }

    _getDefaultAudioSink() {
        return _audioSpawnSync("pactl get-default-sink") || "";
    }
    ...
    _audioPortType(portName) {
        let p = portName.toLowerCase().replace(/^\[out\]\s*/, "");
        if (p.includes("headphone") || p.includes("headset"))             return "headphone";
        if (p.includes("hdmi") || p.includes("displayport") ||
            p.includes("display port") || p.includes("digital-output-hdmi") ||
            p.startsWith("hdmi-output"))                                   return "hdmi";
        ...
        if (p.includes("lineout") || p.includes("line-out") ||
            p.includes("line_out") || p.includes("analog-output-lineout")) return "lineout";
        return "analog";
    }

    _audioIconNameFor(sType, pType) {
    ...
    _audioDisplayNameFor(sink, port, sType) {
        if (sType === "bluetooth") return sink.desc || "Bluetooth";
        if (sType === "usb")       return sink.desc || "USB-Audio";
        if (port) {
            let pt = this._audioPortType(port.name);
            if (pt === "headphone") return _("Headphones");
            if (pt === "lineout")   return _("Speaker (Line-out)");
            if (pt === "speaker")   return _("Speaker");
    ...
    _switchAudioOutputTo(sink, port) {
        if (port)
            GLib.spawn_command_line_async("pactl set-sink-port " + sink.id + " " + port.name);
        GLib.spawn_command_line_async("pactl set-default-sink " + sink.id);

        // Aktive Streams auf den neuen Sink verschieben
        let inputs = _audioSpawnSync("pactl list short sink-inputs");
        if (inputs) {
            for (let line of inputs.split("\n")) {
                let id = line.trim().split(/\s+/)[0];
                if (id && /^\d+$/.test(id))
                    GLib.spawn_command_line_async("pactl move-sink-input " + id + " " + sink.id);
            }
        }
    ...
    _buildAudioOutputMenu() {
        this._audioOutputItem.menu.removeAll();

        let sinks       = this._parseAudioSinks();
        let defaultSink = this._getDefaultAudioSink();

        if (sinks.length === 0) {
            this._audioOutputItem.menu.addMenuItem(
                new PopupMenu.PopupMenuItem(_("No audio outputs found"))
            );
            return;
        }

        for (let sink of sinks) {
            let sType = this._audioSinkType(sink.name);

            if (sType === "bluetooth" || sType === "usb" || sink.ports.length === 0) {
                let pType    = this._audioPortType(sink.activePort);
                let icon     = this._audioIconNameFor(sType, pType);
                let label    = this._audioDisplayNameFor(sink, null, sType);
                ...
                item.connect("activate", () => this._switchAudioOutputTo(sink, null));
                this._audioOutputItem.menu.addMenuItem(item);
                continue;
            }

            for (let port of sink.ports) {
                let pType    = this._audioPortType(port.name);
                let icon     = this._audioIconNameFor(sType, pType);
                let label    = this._audioDisplayNameFor(sink, port, sType);
                ...
                item.connect("activate", () => this._switchAudioOutputTo(sink, port));
                this._audioOutputItem.menu.addMenuItem(item);
            }
        }
    }

    _refreshAudioOutputItem() {
        let sinks       = this._parseAudioSinks();
        let defaultSink = this._getDefaultAudioSink();
        let active      = sinks.find(s => s.name === defaultSink);

        if (active) {
            let sType = this._audioSinkType(active.name);
            let port  = active.ports.find(p => p.name === active.activePort) || null;
            let label = this._audioDisplayNameFor(active, port, sType);
            this._audioOutputItem.label.set_text(_("Audio Output") + ": " + label);
        } else {
